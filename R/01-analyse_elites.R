...
#import data and separate by distance measure
...
aictab(list(c.one, c.two, c.int, c.age, c.sex, c.education, c.age_sex_education),
       modnames = c('one-way', 'two-way', 'interaction',
...
tukey.c.two
eta_squared(c.age_sex_education)

n.two <- aov(distance_score ~ Group+TargetEmotion, n_df) #two-way
plot(n.two)
par(mfrow=c(1,1))
summary(n.two)
...
c.lmm.controls <- lmer(distance_score ~ group+target_emotion+years_of_education+wasiiq+
                (1|id), c_df)
c.lmm <- lmer(distance_score ~ group+target_emotion+
                     (1|id), c_df)
c.lmm.noid <- lmer(distance_score ~ group+(1|target_emotion), c_df)
